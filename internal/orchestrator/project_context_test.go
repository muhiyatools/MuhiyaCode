...
// TestRenderProjectContextBlockUserInstructions covers the user-level file (B1
// T064): user-only omits the override note; user+project adds it and orders user
// before project.
func TestRenderProjectContextBlockUserInstructions(t *testing.T) {
	u := RenderProjectContextBlock("UH", "global user rule", "", "", "", "")
	if !strings.Contains(u, `<user-instructions sha256="UH">`) || !strings.Contains(u, "global user rule") {
		t.Fatalf("user-only block missing user section:\n%s", u)
	}
	if strings.Contains(u, "override user instructions") {
		t.Fatalf("no override note should appear without project instructions:\n%s", u)
	}
	both := RenderProjectContextBlock("UH", "user rule", "IH", "project rule", "", "")
	if !strings.Contains(both, "Project instructions override user instructions on conflict.") {
		t.Fatalf("missing override note:\n%s", both)
	}
	if strings.Index(both, "<user-instructions") > strings.Index(both, "<project-instructions") {
		t.Fatalf("user instructions must precede project instructions:\n%s", both)
	}
}
...
func TestSystemPromptProjectContextDeterministicAndDated(t *testing.T) {
	...
	if !strings.Contains(a, "PROJECT MEMORY") || !strings.Contains(a, "## PROJECT CONTEXT") || !strings.Contains(a, "recall_memory") {
